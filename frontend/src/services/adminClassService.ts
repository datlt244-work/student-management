...
): Promise<AdminClassListResponse> {
...
export async function updateAdminClass(
  classId: number,
  request: AdminUpdateClassRequest,
): Promise<AdminClassListItem> {
...
export async function consolidateClasses(semesterId: number): Promise<void> {
  const response = await apiFetch(`/admin/classes/consolidate/${semesterId}`, {
    method: 'POST',
  })
  if (!response.ok) {
    const errorData = await response.json().catch(() => null)
    throw new Error(errorData?.message || `Failed to consolidate classes (${response.status})`)
  }
}
