...
const teacherData: UserDetail = {
  ...
  experience:
    'Over 12 years of teaching experience in higher education. Joined the institution in 2018. Previously held research positions at the Global Math Research Institute.',
  classes: [
  ...
  schedule: [
    {
      day: 'Mon',
      slots: [
        { time: '09:00 AM', course: 'MATH-401', color: 'blue' },
        { time: '02:00 PM', course: 'MATH-201', color: 'orange' },
      ],
    },
...
<template>
  ...
  <div class="max-w-[1400px] w-full mx-auto p-6 md:p-10 flex flex-col gap-6">
    ...
    <nav class="flex items-center gap-2 text-sm font-medium">
      ...
      <span class="text-slate-900 dark:text-white">{{
        isTeacher ? 'Teacher Profile' : 'Student Details'
      }}</span>
    </nav>
    ...
    <div
      ...
      class="rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-900/10 p-4 flex items-center gap-3"
    >
      <span class="material-symbols-outlined text-red-500">error</span>
      <p class="text-red-700 dark:text-red-400 font-medium">{{ fetchError }}</p>
      <RouterLink
        :to="{ name: 'admin-users' }"
        class="ml-auto text-red-600 dark:text-red-400 hover:underline font-semibold"
    ...
    <template v-else-if="!isTeacher">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3 shrink-0">
          ...
          >
            <span class="material-symbols-outlined text-[20px]">{{
              user.status === 'BLOCKED' ? 'how_to_reg' : 'block'
            }}</span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-3 flex flex-col gap-6">
          <!-- Avatar card -->
          <div
            class="bg-surface-light dark:bg-surface-dark p-8 rounded-xl border border-stone-200 dark:border-stone-800 shadow-sm flex flex-col items-center text-center"
          >
            <div class="relative mb-6">
              <div
                class="w-32 h-32 rounded-full border-4 border-stone-50 dark:border-stone-800 overflow-hidden shadow-xl ring-4 ring-primary/10"
              >
                <img
                  v-if="user.avatar"
                  :src="user.avatar"
          ...
          <!-- Quick Summary -->
          <div
            class="bg-surface-light dark:bg-surface-dark p-6 rounded-xl border border-stone-200 dark:border-stone-800 shadow-sm flex flex-col gap-4"
          >
            ...
            <div class="space-y-3">
              ...
              <div class="flex justify-between items-center text-sm">
                ...
                <span class="font-bold text-slate-900 dark:text-white"
                  >{{ user.credits }} / {{ user.creditsTotal }}</span
                >
              </div>
              <div
                class="w-full bg-stone-100 dark:bg-stone-800 h-2 rounded-full overflow-hidden mt-1"
              >
                <div
                  class="bg-primary h-full transition-all"
                  :style="{ width: user.creditsProgress + '%' }"
                ></div>
              </div>
            </div>
          </div>
        ...
        <div class="lg:col-span-9 flex flex-col gap-6">
          <!-- Tabs -->
          <div
            class="bg-surface-light dark:bg-surface-dark rounded-xl border border-stone-200 dark:border-stone-800 shadow-sm overflow-hidden"
          >
            <div
              class="border-b border-stone-200 dark:border-stone-800 bg-stone-50/50 dark:bg-stone-900/30"
            >
              ...
              <div v-show="activeTab === 'personal'" class="space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
                  ...
                  <div class="md:col-span-2">
                    ...
                    <p
                      class="text-slate-900 dark:text-white font-medium leading-relaxed whitespace-pre-line"
                    >
                      {{ user.address }}
                    </p>
                  </div>
                ...
                <div class="mt-12 pt-8 border-t border-stone-100 dark:border-stone-800">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div
                      class="bg-stone-50 dark:bg-stone-900/50 p-5 rounded-lg border border-stone-100 dark:border-stone-800"
                    >
                ...
                <div class="mt-8">
                  <div
                    class="bg-primary/5 p-6 rounded-xl border border-primary/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
                  >
                    <div class="flex items-center gap-4">
                      <div
                        class="w-12 h-12 bg-primary rounded-lg flex items-center justify-center text-white shrink-0"
                      >
          ...
          >
            <div
              class="px-6 py-5 border-b border-stone-200 dark:border-stone-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2"
            >
              <h3 class="text-lg font-bold text-slate-900 dark:text-white">
                Current Semester Courses
              </h3>
              <span
                class="text-xs font-bold px-2.5 py-1 bg-stone-100 dark:bg-stone-800 text-slate-600 dark:text-slate-400 rounded-md uppercase"
              >
                ...
                <tbody class="divide-y divide-stone-200 dark:divide-stone-800">
                  ...
                  >
                    ...
                    <td class="p-4 text-right">
                      <span
                        class="inline-flex items-center rounded-full bg-blue-50 dark:bg-blue-900/30 px-2.5 py-0.5 text-xs font-bold text-blue-700 dark:text-blue-400"
                      >
                        {{ course.status }}
                      </span>
                    </td>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-4 flex flex-col gap-6">
          <div
            class="bg-surface-light dark:bg-surface-dark p-8 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm flex flex-col items-center text-center"
          >
            <div class="relative mb-6">
              <div class="w-40 h-40 rounded-full border-4 border-primary/20 p-1 overflow-hidden">
                ...
                <div
                  v-else
                  class="w-full h-full rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-4xl"
                >
                  {{ (user.name as string)?.substring(0, 2).toUpperCase() }}
                </div>
              </div>
              <span
                class="absolute bottom-2 right-2 w-6 h-6 bg-green-500 border-4 border-white dark:border-surface-dark rounded-full"
              ></span>
            </div>
            ...
            <div class="mt-6 w-full space-y-4 pt-6 border-t border-stone-100 dark:border-stone-800">
              ...
              <div class="flex justify-between items-center text-sm">
                ...
                <span
                  class="inline-flex items-center gap-1.5 rounded-full bg-green-50 dark:bg-green-900/30 px-3 py-1 text-xs font-bold text-green-700 dark:text-green-400 ring-1 ring-inset ring-green-600/20"
                >
                  {{ user.status }}
                </span>
              </div>
            ...
            <div class="mt-8 flex gap-3 w-full">
              <button
                class="flex-1 px-4 py-2 bg-stone-100 dark:bg-stone-800 hover:bg-stone-200 dark:hover:bg-stone-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-semibold transition-all"
              >
                Send Email
              </button>
              <button
          ...
          <div
            class="bg-surface-light dark:bg-surface-dark p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm"
          >
            ...
            <div class="space-y-4">
              <div class="flex items-start gap-4">
                <span class="material-symbols-outlined text-slate-400 text-[20px] mt-0.5"
                  >mail</span
                >
                <div>
        ...
        <div class="lg:col-span-8 flex flex-col gap-8">
          ...
          <div
            class="bg-surface-light dark:bg-surface-dark rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm overflow-hidden"
          >
            <div
              class="p-6 border-b border-stone-100 dark:border-stone-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2"
            >
              ...
              <span class="bg-primary/10 text-primary text-xs font-bold px-3 py-1 rounded-full"
                >{{ (user.classes as unknown[])?.length ?? 0 }} Active Classes</span
              >
            </div>
            <div class="overflow-x-auto">
                ...
                <tbody class="divide-y divide-stone-100 dark:divide-stone-800">
                  ...
                  >
                    ...
                    <td class="px-6 py-4 text-right">
                      <a
                        class="text-primary hover:text-primary-dark font-bold text-sm cursor-pointer"
                        >Manage Class</a
                      >
                    </td>
                  </tr>
          ...
          >
            <div
              class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-6"
            >
              <div class="flex items-center gap-3">
                <div class="p-2 bg-primary/10 rounded-lg text-primary">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-5 gap-4 overflow-x-auto">
              <div
                v-for="day in user.schedule as {
                  day: string
                  slots: { time?: string; course?: string; type?: string; color?: string }[]
                }[]"
                :key="day.day"
                class="flex flex-col gap-3 min-w-[80px]"
              >
                <span class="text-[10px] font-bold text-slate-400 uppercase text-center">{{
                  day.day
                }}</span>
                <template v-for="(slot, si) in day.slots" :key="si">
                  ...
                  <div
                    ...
                    :class="[
                      ...
                      slot.color === 'orange'
                        ? 'bg-orange-50 dark:bg-orange-900/20 border-primary'
                        : '',
                      slot.color === 'purple'
                  ...
                  >
                    ...
                    <p
                      class="text-[11px] font-semibold text-slate-900 dark:text-white leading-tight mt-0.5 truncate"
                    >
                      {{ slot.course }}
                    </p>
                  </div>
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="showEditModal" class="fixed inset-0 z-[110] flex items-center justify-center p-4">
        ...
        >
          <!-- Header -->
          <div
            class="px-8 py-6 border-b border-stone-100 dark:border-stone-800 flex items-center justify-between shrink-0"
          >
            <div class="flex items-center gap-4">
              <div
          ...
          <form class="flex-1 overflow-hidden flex flex-col" @submit.prevent="submitUpdate">
            <div class="p-8 overflow-y-auto space-y-6">
              ...
              <div
                v-if="editError"
                class="p-3 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-sm flex items-center gap-2"
              >
              ...
              <div
                class="p-4 rounded-xl bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 text-xs text-blue-700 dark:text-blue-400 flex items-start gap-3"
              >
                <span class="material-symbols-outlined text-[18px]">info</span>
                <p>
                  Core account details (Email, Role, Status) are managed by IT administrators and
              ...
              <div v-if="detail?.teacherProfile" class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="flex flex-col gap-1.5 sm:col-span-2">
                  ...
                  >
                  <input
                    v-model="editTeacher.teacherCode"
                    class="py-2.5 px-3 bg-stone-50 dark:bg-stone-800 border-stone-200 dark:border-stone-700 rounded-xl text-sm focus:ring-primary focus:border-primary transition-all"
                    type="text"
                    placeholder="e.g. HJ170001"
                  />
                </div>
                <div class="flex flex-col gap-1.5">
                  <label class="text-xs font-semibold text-slate-600 dark:text-slate-300"
                  ...
                  >
                  <select
                    v-model.number="editTeacher.departmentId"
                  ...
                  <p
                    ...
                    class="text-[10px] text-slate-500 mt-0.5 animate-pulse italic"
                  >
                    Loading departments…
                  </p>
                </div>
                <div class="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    inputmode="numeric"
                    @input="editTeacher.phone = editTeacher.phone.replace(/\D/g, '')"
                    class="py-2.5 px-3 bg-stone-50 dark:bg-stone-800 border-stone-200 dark:border-stone-700 rounded-xl text-sm hover:border-stone-300 dark:hover:border-stone-600 focus:ring-primary transition-all"
                    type="tel"
                  />
                </div>
                <div class="flex flex-col gap-1.5">
                  ...
                  >
                  <input
                    v-model="editTeacher.specialization"
                    class="py-2.5 px-3 bg-stone-50 dark:bg-stone-800 border-stone-200 dark:border-stone-700 rounded-xl text-sm focus:ring-primary focus:border-primary transition-all"
                    type="text"
                  />
                </div>
                <div class="flex flex-col gap-1.5">
                  ...
                  >
                  <input
                    v-model="editTeacher.academicRank"
                    class="py-2.5 px-3 bg-stone-50 dark:bg-stone-800 border-stone-200 dark:border-stone-700 rounded-xl text-sm focus:ring-primary focus:border-primary transition-all"
                    type="text"
                  />
                </div>
                <div class="flex flex-col gap-1.5">
                  ...
                  >
                  <select
                    v-model="editTeacher.officeRoom"
                    class="py-2.5 px-3 bg-stone-50 dark:bg-stone-800 border-stone-200 dark:border-stone-700 rounded-xl text-sm focus:ring-primary focus:border-primary transition-all appearance-none cursor-pointer"
                  >
                    ...
                    <option
                      ...
                      :value="room.roomName"
                      :disabled="
                        !!room.assignedTeacherName && room.roomName !== editTeacher.officeRoom
                      "
                    >
                      {{ room.roomName }}
                      <template
                        v-if="room.assignedTeacherName && room.roomName !== editTeacher.officeRoom"
                      >
                ...
                <div class="flex flex-col gap-1.5 sm:col-span-2">
                  <label class="text-xs font-semibold text-slate-600 dark:text-slate-300"
                    >Degrees / Qualifications</label
                  >
                  <textarea
                    v-model="editTeacher.degreesQualification"
              ...
              <div v-else-if="detail?.studentProfile" class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="flex flex-col gap-1.5">
                  ...
                  >
                  <select
                    v-model.number="editStudent.departmentId"
                    class="py-2.5 px-3 bg-stone-50 dark:bg-stone-800 border-stone-200 dark:border-stone-700 rounded-xl text-sm focus:ring-primary focus:border-primary transition-all appearance-none cursor-pointer"
                  >
                    <option :value="null">— Select department —</option>
                    <option v-for="d in departments" :key="d.departmentId" :value="d.departmentId">
                ...
                <div class="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    inputmode="numeric"
                    @input="editStudent.phone = editStudent.phone.replace(/\D/g, '')"
                    class="py-2.5 px-3 bg-stone-50 dark:bg-stone-800 border-stone-200 dark:border-stone-700 rounded-xl text-sm focus:ring-primary focus:border-primary transition-all"
                    type="tel"
                  />
                </div>
                ...
                <div class="flex flex-col gap-1.5">
                  ...
                  >
                  <input
                    v-model.number="editStudent.year"
                    class="py-2.5 px-3 bg-stone-50 dark:bg-stone-800 border-stone-200 dark:border-stone-700 rounded-xl text-sm focus:ring-primary focus:border-primary transition-all"
                    type="number"
                    min="1"
                    max="10"
                ...
                <div class="flex flex-col gap-1.5">
                  ...
                  >
                  <input
                    v-model="editStudent.manageClass"
                    class="py-2.5 px-3 bg-stone-50 dark:bg-stone-800 border-stone-200 dark:border-stone-700 rounded-xl text-sm focus:ring-primary focus:border-primary transition-all"
                    type="text"
                  />
                </div>
            ...
            <!-- Footer -->
            <div
              class="px-8 py-6 border-t border-stone-100 dark:border-stone-800 bg-stone-50/50 dark:bg-stone-900/20 flex items-center justify-end gap-3 shrink-0"
            >
              <button
                type="button"
                @click="closeEditModal"
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="showBlockModal" class="fixed inset-0 z-[120] flex items-center justify-center p-4">
        ...
        ></div>
        <div
          class="relative bg-white dark:bg-surface-dark w-full max-w-md rounded-2xl shadow-2xl p-6 flex flex-col gap-5"
        >
          <!-- Header -->
          <div class="flex items-center gap-3">
          ...
          <form @submit.prevent="submitStatusChange('BLOCKED')" class="space-y-4">
            <p
              v-if="blockError"
              class="text-sm text-red-500 flex items-center gap-1.5 px-3 py-2 bg-red-50 dark:bg-red-900/20 rounded-lg"
            >
            ...
            <div class="flex items-center justify-end gap-3 pt-2">
              ...
              >
                <span v-if="blockLoading" class="material-symbols-outlined text-[18px] animate-spin"
                  >progress_activity</span
                >
                Confirm Block
              </button>
