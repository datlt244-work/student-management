...
const profileInfo = computed(() => [
  {
    icon: 'badge',
    label: 'Teacher Code',
    value: profile.value?.teacherProfile?.teacherCode ?? 'N/A',
  },
  {
    icon: 'account_balance',
    label: 'Department',
    value: profile.value?.teacherProfile?.department?.name ?? 'N/A',
  },
  {
    icon: 'event_available',
    label: 'Joined Date',
    value: profile.value?.createdAt
...
const statsCards = computed(() => [
  {
    icon: 'military_tech',
    label: 'Academic Rank',
    value: profile.value?.teacherProfile?.academicRank ?? 'N/A',
  },
  {
    icon: 'meeting_room',
    label: 'Office Room',
    value: profile.value?.teacherProfile?.officeRoom ?? 'N/A',
  },
      ...
      <template v-else>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-1">
            <div
              class="bg-surface-light dark:bg-surface-dark rounded-xl border border-border-light dark:border-border-dark p-6 text-center shadow-sm"
            >
              <!-- Avatar -->
              ...
              <p class="text-primary font-medium text-sm">
                {{ profile?.teacherProfile?.academicRank ?? 'Teacher' }}
              </p>
          ...
          <div class="lg:col-span-2">
            <div
              class="bg-surface-light dark:bg-surface-dark rounded-xl border border-border-light dark:border-border-dark shadow-sm flex flex-col h-full"
            >
              <!-- Tabs -->
              ...
              <div v-if="activeTab === 'professional'" class="p-8 flex-1">
                ...
                <!-- Actions -->
                <div
                  class="mt-6 pt-6 border-t border-border-light dark:border-border-dark flex justify-end gap-3"
                >
                  <button
              ...
              <div v-if="activeTab === 'portfolio'" class="p-8 flex-1">
                <div class="text-center py-16 text-text-muted-light dark:text-text-muted-dark">
                  ...
                  <p class="text-sm mt-2">
                    Course history, teaching materials, and publications will be displayed here.
                  </p>
                </div>
              </div>
              ...
              <div v-if="activeTab === 'security'" class="p-8 flex-1">
                <div class="space-y-6">
                    ...
                    <div class="flex flex-col gap-2">
                      ...
                      <div v-if="passwordData.newPassword" class="mt-2 space-y-2">
                        <div class="flex justify-between text-xs font-medium">
                          ...
                          >
                          <span :class="['font-bold', passwordStrength.color]">{{
                            passwordStrength.label
                          }}</span>
                        </div>
                        <div
                          class="h-1.5 w-full bg-border-light dark:bg-border-dark rounded-full overflow-hidden"
                        >
                          <div
                  ...
                  <!-- Security Requirements Checklist -->
                  <div
                    class="bg-background-light dark:bg-stone-800/50 p-4 rounded-lg border border-border-light dark:border-border-dark"
                  >
                ...
                <!-- Actions -->
                <div
                  class="mt-10 pt-6 border-t border-border-light dark:border-border-dark flex justify-end gap-3"
                >
                  <button
                    class="px-6 py-2 rounded-lg bg-background-light dark:bg-stone-800 font-bold text-sm hover:bg-stone-200 dark:hover:bg-stone-700 transition-colors"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <div>
              <p
                class="text-xs text-text-muted-light dark:text-text-muted-dark font-medium uppercase tracking-wider"
              >
                {{ stat.label }}
              </p>
